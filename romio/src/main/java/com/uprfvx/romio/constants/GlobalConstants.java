...
public class GlobalConstants {
    ...
    // Strong attacks (mostly 75+ BP) carrying a real drawback: self-KO, bad accuracy, recharge, or
    // multi-turn lock. The trainer randomizer soft-weights these down (badStrongMoveWeight) so a clean
    // move wins all else equal, without ever banning them.
    public static final List<Integer> badStrongMoves = Arrays.asList(
            MoveIDs.blastBurn, MoveIDs.blizzard, MoveIDs.dragonRush, MoveIDs.dynamicPunch, MoveIDs.eggBomb,
            MoveIDs.explosion, MoveIDs.finalGambit, MoveIDs.focusBlast, MoveIDs.frenzyPlant, MoveIDs.gigaImpact,
            MoveIDs.hurricane, MoveIDs.hydroCannon, MoveIDs.hyperBeam, MoveIDs.inferno, MoveIDs.ironTail,
            MoveIDs.magmaStorm, MoveIDs.megaKick, MoveIDs.outrage, MoveIDs.petalDance, MoveIDs.roarOfTime,
            MoveIDs.rockWrecker, MoveIDs.selfDestruct, MoveIDs.slam, MoveIDs.thrash, MoveIDs.thunder,
            MoveIDs.uproar, MoveIDs.zapCannon
    );
